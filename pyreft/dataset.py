...
def get_intervention_locations(**kwargs):
    ...
    pad_mode = kwargs.get("pad_mode", "first")
    ...
    if share_weights or (first_n == 0 or last_n == 0):
        position_list = list(range(first_n)) + \
            list(range(last_position - last_n, last_position)) + \
        ...
        left_intervention_locations = list(range(first_n)) + [pad_position for _ in range(left_pad_amount)]
        right_intervention_locations = list(range(last_position - last_n, last_position)) + \
    ...
    def load_dataset(self):
        ...
        if self.dataset is None:
            ...
            if self.data_path is None:
                task_dataset = load_dataset(self.task, split=self.data_split, trust_remote_code=True)
            elif self.data_path.endswith(".json"):
                task_dataset = load_dataset("json", data_files=self.data_path, split="train", trust_remote_code=True)
            else:
                task_dataset = load_dataset(self.task, self.data_path, split=self.data_split, trust_remote_code=True)
        else:
